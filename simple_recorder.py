...
"""
    ...
    def _run_batch_diarization(self, audio_path: Path, transcript_data: dict) -> dict:
        ...
        try:
            ...
            # Rebuild transcript text with speaker labels
            speaker_names = config.get_speaker_names()
            label_map = diarizer._build_label_map(diarization_segments, speaker_names)

            # Re-label the plain transcript text using diarization timing
            lines = transcript_data["transcript_text"].split("\n")
            # The transcript_text is a simple joined string — rebuild from segments if available
    ...
    def _rebuild_transcript_with_speakers(
        ...
        # Parse timestamps from lines like "[3.00s] [Other]: text"
...
def _run_post_recording_diarization(segments: list, app_config, recorder) -> list:
    ...
    # Look for saved system audio file
...
def record(duration, session_name):
    ...
    def on_transcript_segment(segment):
        ...
        partial_marker = " [partial]" if segment.is_partial else ""
        print(f"  [{segment.speaker}]: {segment.text}{partial_marker}", flush=True)
    ...
    def signal_handler(signum, frame):
        ...
        # Stop incremental diarization first
        ...
        if transcriber and recording_started:
            try:
                ...
                # Run post-recording diarization if enabled and audio was saved
...
def get_diarization_config():
    ...
    config = get_config()
    diar_config = config.get_diarization_config()
    # Don't expose the full token in output
